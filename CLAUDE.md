...
- **[GATED + DEVICE-PASSED 2026-08-21, awaiting merge decision] Trailer Location (FEAT-25 round one)** — poster-vs-hero playback option on NuvioMobile `claude/trailer-playback-location` (final head `aa57a8d6`, off beta.14 head `dcd84a69`); sim-build CI green, Codex 5 rounds clean, test01+test37 green on the sim, device pass items 3–8 passed (item 8 with a note). Pointer bumped on outer branch `claude/trailer-playback-location-option-lwpsd0` only — merging to `main`/`tvos-shared-extraction` is Christian's call. Three follow-ups recorded in `docs/trailer-playback-location-plan.md` (CW/Upcoming mute, latch-flip collapse animation, refused-slot retry).
