...
- **Phase 5b — Collections, browse-only scope. ✅ SHIPPED 2026-07-02 (verified on device; 1 fix round — Kotlin `Collection`/module name collision).** Key discovery: **collections sync was already live on tvOS** — `SyncManager.pullAllForProfile` (C3) calls `CollectionSyncService.pullFromServer`, so mobile-curated collections were landing on the Apple TV unrendered. tvOS now renders them: collection folder-tile rows on Home (interleaved with catalog rows per Home Rows settings order via `HomeCatalogSettingsRepository.syncCollections` + a `HomeRow` enum in HomeViewModel), and a `FolderDetailView` tabbed paginated grid over shared `FolderDetailRepository` (tab chips per source + All, infinite scroll, poster → Detail). Editing stays on mobile (chosen scope). Pure Swift, no relink. Deferred: tvOS editor/source-picker, ROWS/FOLLOW_LAYOUT view modes (all render as tabbed grid), focus GIFs, hero backdrop/video.
