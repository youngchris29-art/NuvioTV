...
- **[MERGED INTO beta.14 2026-08-21] Trailer Location (FEAT-25 round one)** — poster-vs-hero playback option; gated (Codex 5 rounds clean, sim green, device pass 3–8 passed) and fast-forwarded into NuvioMobile `claude/beta14` AND `tvos-shared-extraction`, both at `aa57a8d6`, then FEAT-26 (bigger season posters, `f539eafd`) merged on top → `65c486f7`. Ships in beta.14. Record + three follow-ups (CW/Upcoming mute, latch-flip collapse animation, refused-slot retry) in `docs/trailer-playback-location-plan.md`; beta.14 entry in `docs/beta14-implementation-plan-2026-08-21.md`.
