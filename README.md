...
- **Native SwiftUI** built around the tvOS focus engine — poster cards scale, lift and tilt toward you on focus as one piece, artwork edge and title together (portrait or wide 16:9 layouts, titles always clear), D-pad-first navigation throughout. Prefer stillness? **No Zoom on Focus** (Settings → Appearance) marks the focused card with a border and shadow instead of scaling — and it covers every tile (See All cards, episode cards, the detail trailer thumbnail), not just posters. See All grids **and collection folder grids** remember your position when you back out of a title, and expanding a search row's See All shows the actual search results. New in this beta: the **Large poster size finally lays out cleanly** — rows land in the same place every time, no posters cut off, no titles overlapping the row above — and No Zoom on Focus behaves the same on every tile, every time.
...
- **Floating pill navigation bar**, horizontal episode shelves, and watched-episode badges. **Bigger season posters** on the series page — 180×270, the same size as More Like This, with the same card depth as every other tile. New in this beta: when TMDB has no season art, the season row uses the **addon's own season posters** (specials included, mapped to the right season), and addons that publish a **localized age rating** get it shown in the rating chip.
...
- **Skip Intro / Recap / Outro** and **Play Next Episode** as Apple's own contextual-action pills (bottom-right, focusable) with a small countdown caption above them, next-episode autoplay, and the Info tab's live stream rows (engine, codecs, resolution, active audio/subtitle, bitrate, transfer). The mpv player draws matching chips. New in this beta: **anime skip segments resolve through Simkl** (the retired ARM service is gone), mapping each episode to the right season so multi-season anime get the correct intro/outro timings; **Menu dismisses the up-next countdown** instead of leaving the player (press again to exit), with a matching **Dismiss** action next to Play Next Episode in the native player; and next-episode auto-play **honors your auto-play source scope** (installed add-ons only / enabled plugins only) — it picks as soon as the sources you chose have answered instead of waiting on the rest.
