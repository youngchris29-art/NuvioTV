...
- **Phase 7 — Trakt (M–L).** Needs `TRAKT_CLIENT_ID/SECRET` **and code**: shared `TraktAuthRepository` only implements the OAuth-redirect flow (`onCancelDeviceFlow` is just an alias for cancel — there is no device-code implementation). tvOS needs the Trakt **device-code flow** (`/oauth/device/code` + polling) added to the shared repo, plus a tvOS "enter code at trakt.tv/activate" screen. Scrobbling then comes via shared `TraktScrobbleRepository`.
