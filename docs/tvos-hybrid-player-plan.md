...
**Phase 5 DEVICE-VALIDATED (2026-07-17, Apple TV 4K tvOS 27, user-confirmed): a real DV Profile 7 FEL 2160p BluRay REMUX (Top Gun: Maverick, 2h10m, 11004 keyframes, EAC3 copy audio) plays through the native path with the TV showing the DOLBY VISION badge.** Probe classified FEL live via libdovi; converter engaged; AVPlayer readyToPlay (twice across runs); far-forward scrub (seg 29→132), backward-into-unproduced-hole (142→131) with a scrub-storm coalescing 130→128→127, and a second back-jump (128→104) all produced their targets immediately; process footprint plateaued ~630–650 MB on the high-bitrate remux (vs ~375 MB for the Phase 4 DTS-HD file — well clear of the ~2.1 GB jetsam ceiling). Same session regression-passed the P8.1 path and the TrueHD/DTS→AAC transcode path on device (Inception remuxes — note: Inception's UHD disc is HDR10-only, its remuxes are never P7; hybrids probe as P8.1). Remaining nice-to-have: spot-check a real MEL disc (FEL was the riskier flavor; MEL conversion is identical and fixture-proven) and a full-movie soak.
