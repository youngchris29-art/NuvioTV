...
- **Committed 2026-09-05 00:20 ET** on `claude/steven-beta17`: `55571198` (hero commit protocol), `d66832b6` (row geometry), `9c08d423` (cards, trailers, detail), `ec30496d` (UI gates + fixture). Gate run 4 on that tree: Debug + Release, NuvioTVTests 81, UI 15 targets (14 pass, leg C skip), JVM 702, Kotlin/Native 722.
- **Codex round 4 (post-reset, `--base cf2f674e --scope branch`):** one P2. The image-driven resolver dropped the poster fallback `HeroCrossfadeImage` used to receive, so an IMDb-backed title whose synthesized Metahub backdrop 404s committed a blank hero. Fix in progress as a follow-up commit: title heroes resolve the poster inside the same deadline and use it only when the primary does not land (`present … backdrop=poster`); folder heroes still never fall back to their cover.
- **Codex round 4 fix committed as `1aabcec7`:** title heroes resolve the poster concurrently under the same `.head` admission and deadline and paint it only when the primary does not land (`present … backdrop=poster`); a landed primary always wins, a late primary after a poster commit is dropped, folder heroes are unreachable from the path. NuvioTVTests 87, test20 + test31 legs A/B green. Codex round 5 on the branch in progress.
- **Codex round 5 (branch):** two Kotlin P2s. A committed collection-backed head kept its `COLLECTION_HERO_ORIGIN` sentinel after the collection key changed, so the replacement fallback was never frozen or pinned; and a profile whose launch pull coalesced into a running foreground pull was refused adoption when the previous profile had left the signal `Settled`, so its gate could read that stale `Settled`. Fix agent dispatched (sixth commit); Codex round 6 follows.
- **Codex round 5 fix committed as `266ac919`:** `retainHeroItemOrigins` keeps a collection sentinel origin only while the item is still in the collection cache, so the invalidating publish empties the commit map and the replacement fallback is frozen and pinned like a first commit; `LaunchSyncSignal.claimCoalescedLaunch` takes over a coalesced launch pull whenever the signal's owner is another profile. JVM 708, Kotlin/Native 728. Codex round 6 in progress.
- **Codex round 6 (branch):** one P2, a trade-off from the Hole E fix: dropping `displayTitle` from the catalog descriptor signature stopped a server rename from pruning every section, but also stopped the rename from reaching the cached section subtitle and the settings `addonName` until an unrelated refresh. Fix agent dispatched (seventh commit: title updates cached metadata without touching the cache identity); Codex round 7 follows.
- **Codex round 6 fix committed as `a61ca4d6`:** the add-on title rides the refresh TRIGGER (`buildHomeCatalogRefreshSignature`) but never the cache identity; `isMetadataOnlyDefinitionChange` answers a rename with a metadata-only republish under the held lock (`withCurrentMetadata` on every published section) instead of a refetch, so names repaint while rows and the pinned hero stay put. Correction recorded: tvOS never rendered `HomeCatalogSection.subtitle` (only the settings `addonName`, which resyncs on Settings appear), so the visible staleness was composeApp's. JVM 719, Kotlin/Native 739, composeApp iOS compile green. Codex round 7 in progress.
- **2026-09-05 02:00 ET:** Codex round 7 blocked by the usage limit until 05:46 and scheduled for the reset. Branch pushed (`origin/claude/steven-beta17` @ `a61ca4d6`, seven commits); outer docs pushed (`origin/main` @ `f9ae32b`). Not merged into `tvos-shared-extraction`, pointer not bumped: the next step is Christian's device pass in Steven's configuration.
- **Codex round 7 (post-reset, branch):** one P1 + two P2s, all Kotlin. P1: a held publish forced `isLoading = true` and the release re-derived it, so a profile whose catalogs failed or came back empty stayed on "Loading catalogs…" forever. P2s: the request-time launch-signal claim ran outside the request gate's lock (a rapid profile switch could let the older request overwrite the newer profile's `Running`), and a stale coalesced request could publish `NotApplicable` over a newer profile's claim. Fix agent dispatched (eighth commit); Codex round 8 follows.
- **Codex round 7 fix committed and pushed as `d9a5b1ab`:** `HomeRepository.catalogLoadInProgress` is the real load state and the `isLoading` parameter is gone from the publish path (no caller can echo the forced value back), so a release carries the real state and the preserved error; the launch-signal claim runs inside the request gate's lock (lock order gate → signal everywhere, no reverse path), a coalesced request is handed its absorbing job inside the same critical section, and a claim is refused once another profile owns `Running`. JVM 725, Kotlin/Native 745. Codex round 8 in progress.
- **Codex round 8 (branch):** one Swift P2: the tvOS add-on-change signature hashed only manifest URLs, so a cloud rename never reached the metadata refresh path Compose takes (tvOS renders no section subtitle and the settings label resyncs on Settings appear, so the visible impact was small). Fix agent dispatched (ninth commit: title in the signature, rename-only changes routed to the non-forced refresh); Codex round 9 follows.
- **Codex round 8 fix committed and pushed as `05d6749e`:** `AddonChangeRoute` (none / metadataOnly / refresh) — the tvOS signature carries the display title, a rename-only change syncs catalog settings and asks for a non-forced refresh (answered by the shared metadata-only republish), manifest changes keep the forced refresh; probe `addonRoute=`. NuvioTVTests 92. **Open:** test31 leg B now fails on every run with a full row-order reversal, reproduced with this commit's change stashed, so the cause is in the Kotlin commits since gate run 4 (`266ac919`, `a61ca4d6`, `d9a5b1ab`) or in the oracle only now observing the burst's legitimate post-commit reorder. Investigator dispatched (tenth commit if code, oracle fix if not); Codex round 9 waits on it.
- **Leg B verdict (commit `46a0d9b7`, pushed): oracle, not code.** The burst's reversal was always post-release; whether it lands before or after the commit is a race on hero-art warmth (23:48 green: `released:timeout`, commit at 8157 ms after a 1382 ms prewarm absorbed the reversal into the gated rebuild; 07:05 red: `released:all`, commit at 5457 ms after a 9 ms prewarm, reversal 994 ms later). `RowsOrderRule` now excuses a reorder only when it is after the first commit, `settingsSig=` (new `rows` token) moved between the two lines, and no second commit or `headChanged=1` sits between; older lines without `settingsSig` are judged as before. test31 green twice with the reversal present and excused. **New open item from the same run:** with a 40-press budget leg C reaches a folder tile and then finds no `present item=nuvio.folder:…` line at all; either the fixture's folders carry no art (documented skip reason), the hero-follows-folder path does not drive the resolver (a Wave H regression that would hit Steven's Home directly), or the oracle greps the wrong id shape. Investigator dispatched; Codex round 9 running in parallel.
- **Codex round 9 (branch):** one P1 the internal review had missed: on a cold launch the add-on watcher's first emission is the repository's initial EMPTY state (before `initialize()` runs), and the no-sources escape treated it as terminal and opened the rows gate on every cold launch, so the collections and settings watchers could paint rows before the hero commit. Fix agent dispatched (eleventh commit: escape requires add-on bootstrap to have settled, `bootstrap=` probe token, pure `AddonBootstrapRoute` seam); folder-hero investigation still running in parallel; Codex round 10 after both.
- **Codex round 9 fix committed and pushed as `d335d573`:** `AddonsUiState.isInitialized` (set on all three bootstrap outcomes), `AddonBootstrapRoute` seam (pre-initialize → hold rows; initialized + empty + nothing pending → open as no-sources; refreshed already → hold), `bootstrap=<pre|settled>` probe token. Correction recorded: the initial EMPTY state was already intercepted by the seed guard one branch earlier; the reachable pre-bootstrap shape is a non-empty state published before `initialize()`, now closed for every shape. Standing bug found outside scope and filed as a task chip: `pullFromServer` sets the private `initialized` flag without the `_initializedState` mirror, so a pull that beats `initialize()` leaves that flow false forever (`SubtitleRepository` awaits it). NuvioTVTests 97, test31 legs A/B green with the first `rows` line at the commit and none before it.
- **Folder-hero verdict (commit `c0a6acc3`, pushed): the path works; the oracle read an evicted buffer.** The fixture's folders all carry backdrop + logo (`[CollectionCover] heroBackdrop=1 logo=1`); focusing one produced exactly `present item=nuvio.folder:… backdrop=fetched logo=fetched waited=98 same=0` and one matching `paint`. Reaching a collection row on the burst-reversed 35-row Home costs ~40 Down presses and the walk back another ~40 Up, each emitting a present/paint pair, so ~150 probe lines landed after the folder's and the 32-line rolling tail had evicted it before leg C read the About pane. Leg C now dwells 3 s and asserts live off the resolver via three new append-only `debug_hero` fields (`pitem`, `pbd`, `plg`); the About-pane pairing check skips with a note only when the buffer reports elision. Leg C green twice; test31 green in the combined gate; NuvioTVTests 97. Device pass must still watch the transition itself on Steven's Home: previous hero holds until the folder's backdrop and logo land together, then one swap, no colour block, no black frame, no oversized mosaic shrinking into place.
- **2026-09-05 ~09:00 ET:** Codex round 10 blocked by the usage limit until 10:48 and scheduled for the reset. Branch `claude/steven-beta17` at `c0a6acc3` (12 commits), pushed; outer `main` pushed. Review record: nine Codex rounds plus one internal review, every finding fixed and re-reviewed except the last two commits, which round 10 covers. Next: Christian's device pass in Steven's configuration, merge + pointer bump, rc build, DM to Steven, public beta only after his photo meets the contract.
- **Round 9 chip closed (commit `918cd2a1`, pushed):** the `pullFromServer` mirror bug. Both bootstrap facts now live in `AddonBootstrapState` (`begin()` = the `initialize()` reentrancy guard, `complete()` sets guard and mirror on every completion path including both server-pull paths, `reset()` clears both); the mirror still flips only after the list is published. `AddonBootstrapStateTest` (3 cases: pull-before-initialize flips the mirror, begin alone does not, reset clears both). jvmTest 728, tvOS sim compile green.
- **Codex round 10 (branch, `46a0d9b7..918cd2a1`, 10:49 ET):** one P2 in `HomeViewModel.onAddonsChanged`: the empty-list branch seeded Cinemeta and returned before asking `AddonBootstrapRoute`. `addAddon` adds nothing when the manifest fetch fails, so on an offline fresh install no add-on emission ever followed and the rows gate stayed shut for the session (collections-only Home blank). Fixed in the same session, not delegated.
- **Codex round 10 fix committed and pushed as `8619c690`:** `AddonBootstrapRoute` gains a `seedPending` term; the settled empty list now runs the decision (holds while the seed is pending or in flight, opens as no-sources once the seed's completion reports a failure on a still-empty list); a seed attempt counter keeps a stale completion from marking a newer profile's seed as failed; the pre-bootstrap emission still holds. NuvioTVTests 99 (two new route cases). Outer pointer bumped on `claude/epic-snyder-2d920b`. Codex round 11 (`918cd2a1..8619c690`) running.
- **Codex round 10 (post-reset, branch):** one P1 claiming `TrailerZoomProbe`'s locked statics break the build under MainActor default isolation. Premise checked and false: the target is Swift 5 with `SWIFT_APPROACHABLE_CONCURRENCY = YES` and no `SWIFT_DEFAULT_ACTOR_ISOLATION`, and the branch built Debug and Release on every gate run. Taken anyway as a hardening (thirteenth commit: `nonisolated(unsafe)` on the lock-protected storage, matching `HomeHeroProbe`), then Codex round 11 to confirm the loop is closed.
- **Codex round 11 (branch, `918cd2a1..8619c690`): CLEAN.** "The seed-failure state is profile-scoped, stale profile completions are invalidated, and the rows gate remains held until seeding resolves. No actionable regression was found." Branch `claude/steven-beta17` at `8619c690` (14 commits), pushed. Review record: eleven Codex rounds plus one internal review, every finding fixed and re-reviewed. Next: Christian's device pass in Steven's configuration, merge into `tvos-shared-extraction` + pointer bump on `main`, rc build, DM to Steven, public beta only after his photo meets the contract.
- **2026-09-05 11:20 ET, branch at 15 commits, tip `2a5874db`, pushed.** From Christian's parallel session on the same checkout: `918cd2a1` (`AddonBootstrapState` keeps the private `initialized` guard and the `initializedState` mirror in step on every completion path, closing the standing bug found during the round-9 fix) and `8619c690` (an offline fresh install whose Cinemeta seed fails now opens the rows gate as no-sources once the seed's completion reports failure, instead of a blank collections-only Home for the session; `AddonBootstrapRoute` gains a `seedPending` term; NuvioTVTests 99). From this session: `2a5874db` (`nonisolated(unsafe)` on `TrailerZoomProbe`'s locked buffer; premise of the Codex round-10 finding was false, taken as hardening). Codex round 11 runs in Christian's session.
- **2026-09-05 11:30 ET, MERGED.** `claude/steven-beta17` fast-forwarded into `tvos-shared-extraction` (`cf2f674e..2a5874db`, 15 commits, pushed) and the outer pointer bumped on `main`. Device pass in Steven's configuration not yet run; rc build, DM to Steven and his photo still ahead.
- **2026-09-05 16:15 ET, DEVICE PASS PASSED.** Debug build stamped `2a5874db` (bundle 116, dev-signed) installed over 115 on the Living Room ATV via devicectl and run by Christian in Steven's configuration; all four sections passed (hero photo contract, rows probe, focus/trailer/scroll passes 1 and 2, regression spot checks). Build number bumped to 117 for the beta.18 cycle (`146d2233` on `tvos-shared-extraction`, pushed). rc1 for beta.18 cut from that commit as an unsigned Release IPA tagged `tvos-v0.3.0-beta.18-rc1`; DM to Steven still gated on the SlopMonster loop and an explicit go-ahead.
